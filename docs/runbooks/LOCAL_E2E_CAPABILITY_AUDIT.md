...
| **真实播放推进** | currentTime 采样全 0（0→0→0→0），`currentTime_increased=false` | ⚠️ headless 静音自动播放未推进（限流/需人工点播） |
| isPassed | isPassed=null（未读到明确服务端完成标记） | ⚠️ 未观测到 |
...
| **CAP-003b 单章 run（currentTime 推进）** | **PARTIAL** | headless 静音自动播放 currentTime 全 0；说明播放推进在自动化受限 + 需人工/非 muted 触发 | boundary：自动化播放被限，不代表站点未起播 |
| **CAP-004 multimedia/log 上报** | **VERIFIED** | 真实 `multimedia/log` POST 200 出现在 wire（count=1） | 证明完成上报链路真实可达 |
| **CAP-005 server completion（isPassed）** | **UNKNOWN** | 读不到 isPassed（null），未拿到稳定服务端完成标记 | 需真实完整播放，或服务端采样才能判 |
...
PYTHONPATH=. python scripts/local_e2e_audit.py
# → 写 docs/evidence/e2e_local_audit.json（脱敏）
```
> 提醒：这是会触真站/真登录的审计，可能产生真实学习记录、触发验证码/风控，不要在正式环境反复跑。